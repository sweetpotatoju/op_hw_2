...
print(f"Cross-validated MSE (Random Forest Regression): {-rf_scores.mean()}")

# 테스트 데이터셋 예측 및 평가
rf_y_pred = rf_model.predict(X_test)
rf_mse_test = np.mean((y_test - rf_y_pred) ** 2)
print(f"Test MSE (Random Forest Regression): {rf_mse_test}")
...
def find_best_station_for_hour_rf(hour, dew_point, humidity, rainfall, snowfall, solar_radiation, temp, visibility,
    ...
    model_ga = ga(function=fitness_rf, dimension=13, variable_type='real', variable_boundaries=varbound,
...
for hour in range(24):
    best_station = find_best_station_for_hour_rf(hour, dew_point=5, humidity=60, rainfall=0, snowfall=0,
